...
function ManageBook() {
  ...
  if (Object.keys(member).length > 0) {
    checkOutBooks = member.check_out_members.filter((checkOut) => {
    ...
    missedDeadlines = member.check_out_members.filter((checkOut) => {
      if (checkOut.returned_date) {
        return !isBefore(
    ...
    reservedBooks = member.member_reservations;
  }
  ...
  return (
    <>
      ...
      <Box
        sx={{
          width: '100%',
          px: { xs: '10px', sm: '20px', md: '30px', lg: '40px' },
          py: '10px',
        }}
      >
        <ManageBookTable member={member} />
      </Box>
    </>
  );
  // <Box
